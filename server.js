...
socket.on('game_start',function(payload) {
	...
	var requested_user = payload.requested_user;
	if(('undefined' ===typeof requested_user) || !requested_user) {
		var error_message = 'game_start didn\'t specify a message, command aborted';
		log(error_message);
		socket.emit('game_start_response', {
			result:'fail',
			message: error_message
...
socket.on('play_token',function(payload) {
    ...
    /* Check that the play has previously registered */
 ...
 * and if the line indicated by adding dr to r and dc to c eventually ends in
 ...
 function valid_move(who, dr,dc,r,c,board) {
    ...
    if(who === 'b') {
    	...
    	log('Houston we have have a color problem: '+who);
    	return false;
    }
    if( (r+dr < 0) || (r+dr > 7) ) {
    	return false;
...
function flip_line(who,dr,dc,r,c,board) {
	/* Check that you don't walk off edge of board */
	if( (r+dr < 0) || (r+dr > 7) ) {
    	return false;
    }
    if( (c+dc < 0) || (c+dc > 7) ) {
